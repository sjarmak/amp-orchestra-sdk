...
impl CliBackend {
    ...
    fn extract_session_id(&self, path: &str) -> Option<String> {
        ...
        // Try to canonicalize both paths to handle symlinks
        let canonical_path = path_buf.canonicalize().unwrap_or(path_buf.clone());
        let canonical_worktrees_dir = self.worktrees_dir.canonicalize().unwrap_or(self.worktrees_dir.clone());
        
        if let Ok(relative) = canonical_path.strip_prefix(&canonical_worktrees_dir) {
...
impl GitBackend for CliBackend {
    ...
    async fn list_worktrees(&self) -> GitResult<Vec<WorktreeInfo>> {
        ...
        // Split output into worktree entries (separated by empty lines)
        let lines: Vec<&str> = output.lines().collect();
        let mut i = 0;
        
        while i < lines.len() {
            if lines[i].starts_with("worktree ") {
                // Process the previous worktree if any
                ...
                current_worktree = Some(lines[i][9..].to_string()); // Skip "worktree "
                current_branch = None; // Reset branch for this worktree
                
                // Look ahead for branch line
                let mut j = i + 1;
                while j < lines.len() && !lines[j].starts_with("worktree ") && !lines[j].is_empty() {
                    if lines[j].starts_with("branch ") {
                        let branch_ref = &lines[j][7..]; // Skip "branch "
    ...
    async fn cleanup_worktree(&self, session_id: &SessionId) -> GitResult<()> {
        ...
        // 1. Find the actual branch for this worktree before removing it
        ...
        // 3. Delete the branch that was actually used
