...
export const RepositoryNode: React.FC<RepositoryNodeProps> = ({
  ...
  const handleCreateSession = async (name: string, useWorktree: boolean, environment?: SessionEnvironment) => {
    const repoId = repository.id;          // for state
    const repoPath = repository.path;      // for git worktree
    console.log('RepositoryNode.handleCreateSession called with:', { repoId, repoPath, name, useWorktree, environment });
    ...
    try {
      ...
      const sessionId = await createSession(repoId, name, environment || 'production', branch, undefined, repoPath, useWorktree);
