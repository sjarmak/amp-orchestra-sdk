...
export function loadAmpAuthConfig(): AmpAuthConfig {
  ...
  // Do not auto-set AMP_URL; respect environment. Local CLI should talk to production unless explicitly configured.

  return {
    ampBin,
    ampArgs: process.env.AMP_ARGS,
    ampAuthCmd: process.env.AMP_AUTH_CMD,
    ampToken: process.env.AMP_API_KEY || process.env.AMP_TOKEN, // Support both for backward compatibility
...
export async function ensureAmpAuth(): Promise<{ success: boolean; message: string }> {
  ...
  if (!config.ampAuthCmd && !config.ampToken) {
    if (config.debug) console.log('[AMP_DEBUG] Error: Neither AMP_AUTH_CMD nor AMP_API_KEY configured');
    return { success: false, message: 'Neither AMP_AUTH_CMD nor AMP_API_KEY configured' };
  }
  ...
  if (config.ampAuthCmd) {
    ...
    const authEnv: Record<string, string> = config.ampToken ? { AMP_API_KEY: config.ampToken } : {};
